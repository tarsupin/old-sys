...
-----------------------------------------------
------ About the Database_Transfer Class ------
-----------------------------------------------

This class allows you to transfer rows data between tables that are identical in table structure. This is not designed (or optimized) for cloning large sets of data. It's primary function is to backup data in multiple locations, or to prune data from one table and store it in another.

For example, if your forum only wants to load posts that are less than 90 days old, you can prune the old posts and back them up into another table for safekeeping.

This system can also be used for vertical partitioning, which is used to increase performance. For example, every year you could run an algorithm that saved the next set of data into tables like this:
...
This class uses SQL commands to identify which tables you would like to transfer. The method is simple to use:
	...
	Database_Transfer::move($sourceTable, $destinationTable, $sqlWhere, $sqlInput);
...
For example, if you wanted to move all old posts to a backup table, it might look like this:
	...
	Database_Transfer::move($sourceTable, $destinationTable, $sqlWhere, $sqlInput, $sqlLimit);
...
Database_Transfer::copy($sourceTable, $destinationTable);							// copies the entire table
Database_Transfer::copy($sourceTable, $destinationTable, $sqlWhere, $sqlInput);	// copies specific rows to the destination

Database_Transfer::move($sourceTable, $destinationTable);							// moves the entire table
Database_Transfer::move($sourceTable, $destinationTable, $sqlWhere, $sqlInput);	// moves specific rows to the destination

*/

abstract class Database_Transfer {
...
/****** Transfer data from one table to another ******/
	...
	// Database_Transfer::copy($sourceTable, $destinationTable, $sqlWhere, $sqlArray, [$limit], [$move]);
	{
		// Protect Tables
...
/****** Transfer data from one table to another ******/
	...
	// Database_Transfer::move($sourceTable, $destinationTable, $sqlWhere, $sqlArray, [$limit]);
	{
		return Database_Transfer::copy($sourceTable, $destinationTable, $sqlWhere, $sqlArray, $limit, true);
	}
