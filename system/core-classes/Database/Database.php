...
/****** Initialize the Database ******/
	...
	// Database::initialize($databaseName);
	{
		...
		if(!$databaseName and self::$databaseName)
		{
		...
		try
		{
			...
			*/
			...
			$db = Config::get("Database-" . ucfirst(ENVIRONMENT));
			
			// Connect to the database as an admin
			if($asAdmin)
			{
				self::$database = new PDO($db['Engine'] . ":host=" . $db['Host'] . ";dbname=" . $databaseName . ";charset=utf8", $db['Admin Username'], $db['Admin Password'], $options);
			}
			
			// Connect to the database as a standard user
			else
			{
				self::$database = new PDO($db['Engine'] . ":host=" . $db['Host'] . ";dbname=" . $databaseName . ";charset=utf8", $db['Username'], $db['Password'], $options);
			}
