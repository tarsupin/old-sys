...
Text_Format::safe($text)					// Changes illegal characters to safe HTML equivalents
Text_Format::overprotective($text);			// Changes basically everything to safe HTML equivalents
Text_Format::convertWindowsText($text);
...
# Special thanks to anonymous "info" poster on PHP.net
	...
	// $safeText = Text_Format::safe("<אבגדהוז¼½> and other such characters.");
	{
		$newText = "";
		$textLen = strlen($text);
...
# This system prevents basically every type of character, including things like ! and #
	...
	// $safeText = Text_Format::overProtective("<אבגדהוז¼½> and other such characters.");
	{
		$newText = "";
		$textLen = strlen($text);
...
/****** Convert Windows Text ******/
	...
	// $fixText = Text_Format::convertWindowsText($text);
	{
