...
--------------------------------------------
------ About the Text_Utilities Class ------
--------------------------------------------
...
$argString = Text_Utilities::convertArrayToArgumentString($arguments, $maxLengthOfString);

$indexNum = Text_Utilities::strToIndex($string, [$strength], [$charList], [$lowercase]);

*/

abstract class Text_Utilities {
	
	
/****** Get a word list from designated text ******/
	public static function getWordList
	(
		$text				// <str> The text to get the word list of.
	,	$extraChars = ""	// <str> A string of characters to allow in words (other than letters and numbers).
	)						// RETURNS <void>
	
	// $words = Text_Utilities::getWordList($text, [$extraChars]);
	{
		// Separate into words
		$wordList = array();
...
/****** Create Dropdown Options from Array ******/
	...
	(
		$options = array()	// <str:str> The list of options to add to the dropdown.
	,	$selected = ""		// <str> The value to select in the dropdown.
	)						// RETURNS <str> The HTML for the dropdown.
	
	// $dropdownOptions = Text_Utilities::createDropdownOptions($options, $selected);
	{
		$html = '';
...
/****** Convert an array to an "argument" (parameter) string ******/
	...
	// $argString = Text_Utilities::convertArrayToArgumentString(array("hello", 100));
	{
		// Prepare values
		$argDisp = "";
		$comma = "";
