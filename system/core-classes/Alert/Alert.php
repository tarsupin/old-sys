...
Determining if the form has passed successfully or not can be done Alert::hasErrors(), as long as you have made sure that everything that can fail has been marked with an Alert::error() value. The Validate::pass() method uses the Alert class to identify if it has passed or not.
...
// Displays an alert on the NEXT AVAILABLE alert display (often after you redirect to a new page)
...
# To avoid repetition of code, Alerts are called with dynamic names, including Alert::success(), Alert::error(), etc.
	...
	(
		$func		// <str> The name of the function being called
	,	$args		// [int:mixed] Additional arguments being passed
	)				// RETURNS <mixed> value that depends on the function being called
...
/****** Add an Alert ******/
	...
	(
		$type			// <str> The type of alert (success, error, warning, info, etc)
	,	$key			// <str> The key name of the alert
	,	$message		// <str> The alert message (e.g. "You successfully logged in!")
	,	$severity = 0	// <int> Severity (0 to 10); 0 = don't track, 1 = harmless, 10 = obvious hacking attempt.
	,	$save = false	// <bool> TRUE will save this value for the next page load, FALSE is this page only.
...
# Note: This method can be used to identify if a form succeeds or not.
	...
	(
		$key = ""		// <str> If specified, only checks this particular key for errors.
	)					// RETURNS <bool> TRUE if there are error alerts, FALSE if not
...
/****** Display Relevant Alerts ******/
	...
	)		// RETURNS <str> HTML with divs that provide a simple display of relevant alert data
