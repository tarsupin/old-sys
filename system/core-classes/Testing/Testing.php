...
To create an integrity test for a plugin, create a {PLUGIN}.test.php file in the /PLUGIN_DIRECTORY/Testing/tests folder of your plugin. For example, if the name of the plugin is "MyClass", your testing file would be named like this:
	
	/PLUGIN_DIRECTORY/Testing/tests/MyClass.test.php
...
/****** Class Variables ******/
	public $class = "";			// <str> The name of the plugin that you're testing.
	public $lastIndex = 0;			// <int> The last index used in the results array.
	public $lastMethod = "";		// <str> The method used in the last integrity test.
	public $lastResult = null;		// <mixed> The result of your most recent integrity test.
	public $results = array();		// <str:[int:array]> The results that you're testing.
	
	
/****** Initialize the testing plugin ******/
	public function __construct
	(
		$class		// <str> The name of the plugin that you intend to run integrity tests on.
	)				// RETURNS <void>
	
	// $test = new Testing($class);
	{
		$this->plugin = Sanitize::variable($class);
...
/****** Run an Integrity Test ******/
	...
	,	$args		// <int:mixed> The arguments passed into the method being called.
	)				// RETURNS <mixed>
	...
	// $test->{method}([..$arg], [..$arg], ...)
	{
		...
		$checkClass = new ReflectionClass($this->plugin);
		...
		{
			$testClass = new $this->plugin();
			$result = call_user_func_array(array($testClass, $method), $args);
		}
...
/****** Run an Integrity Test ******/
	...
	(
		$expected	// <mixed> The expected result of your last integrity test.
	)				// RETURNS <this>
...
/****** Set an error message if the test fails ******/
	...
	(
		$message	// <str> The message to announce if the test fails.
	)				// RETURNS <this>
	...
	// $test->displayTestResults();
	{
		...
		foreach($this->results as $method => $testList)
		{
			foreach($testList as $round)
			{
				...
				$argString = Text_Utilities::convertArrayToArgumentString($args);
