...
There are two ways to call APIs: a private API, and a public API. If you connect with a private API that you don't have a shared key with, UniFaction's Auth system will attempt to create a set of shared keys between the two sites. To call a private API you only need to run one line:
	...
	$response = API_Connect::to($siteHandle, $apiName, $dataToSend);
...
For example, your API may look like this:
	...
	$response = API_Connect::to("unifaction", "API_IsConnected", "hello!");
...
Calling public APIs requires the use of the API_Connect::call() method. You must enter the full URL path to the public API. For example:
	
	$packet = "ping";
	$response = API_Connect::call(URL::unifaction_com() . "/api/PingPong", $packet);
...
To access the alerts that are sent by the API, just refer to the API_Connect::$alert value after the response has been gathered. For example:
	...
	if($response === false)
	{
		...
		{
			echo "The API encountered the following error: " . API_Connect::$alert;
		}
	}
...
For example, if you want to pass a file and use POST instead of GET:
	...
	$response = API_Connect::to($siteHandle, $apiName, $dataTosend, $settings);
...
To debug an API, you'll often need to know the data that was sent. Doing that will require you to use the URL that you sent. To retrieve the URL that was loaded, you can use the API_Connect::$url value. For example:
...
// This method grants more control than API_Connect::to, but is more complicated to use
$response = API_Connect::call($apiFullURL, $apiData, $apiKey, [$settings]);

*/

abstract class API_Connect {
...
/****** An ALL-IN-ONE call handler of an API ******/
	...
	// $response = API_Connect::call("http://example.com/api/this-api", $dataToSend, [$apiKey], [$settings]);
	{
