...
This plugin is used to create or handle APIs that other phpTesla sites can interact with. Other sites will connect to this API using the API_Connect:: plugin.
...
For example, here is a simple working example of the "PingPong" API Class:
...
And to call the "PingPong" API Class:
	
	// Call the "PingPong" API plugin on "unifaction"
	API_Connect::$isPrivate = false; // because this is a public API
	$response = API_Connect::to("unifaction", "PingPong", "ping");
...
You can add your API Class to your site like any other plugin, either in your system plugins or your application plugin.
...
If you want to return an alert message with the API (generally an error, but doesn't have to be), you can simply set $this->alert to the message you want to return. The Connect plugin will be able to read it with API_Connect::$alert;
...
/****** Class Variables ******/
	...
	public $encryptType = "";			// <str> The type of encryption to respond with. Or "" for no encryption.
	...
	// Connection Data
	public $data = "";			// <mixed> The API data that was sent.
	public $apiHandle = "";		// <str> The site handle that connected to the API.
	public $apiClearance = 0;	// <int> The clearance level of the site using this API.
	public $apiKey = "";		// <str> The API key that is being used to connect.
	public $apiSalt = "";		// <str> The API salt that is being used to connect.
	public $apiConf = "";		// <str> The API confirmation value to prove the encryption wasn't tampered.
	
	// Alerts
	public $meta = array();		// <str:mixed> Mixed metadata that can be sent along with the data (non-encrypted)
	public $alert = "";			// <str> If set, this will send an alert along with the API response.
	
	
/****** Run the Full API, including interpreting the response ******/
	public function __construct (
	)					// RETURNS <mixed> the interpretation of the message
	...
	// $dataReceived = new API();
	{
		...
		if($this->isPrivate)
		{
			...
			// Get the API Data
			$listen = Security_Decrypt::run($this->apiKey . $this->apiSalt, $this->data);
			
			// Make sure we pass the confirmation test (to prove that the data wasn't tampered)
			if($this->apiConf != Security_Hash::value($this->apiKey . $this->apiSalt . $listen, 20))
			{
		...
		{
			$packet['resp'] = Security_Encrypt::run($this->apiKey . $this->apiSalt, json_encode($response), $this->encryptType);
			$packet['enc'] = true;
		}
		else
		{
...
/****** Confirm that this API is safe to use (hasn't already been activated) ******/
	...
	)					// RETURNS <bool> TRUE if the API is safe to use, FALSE if it's already been used.
