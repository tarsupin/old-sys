...
/****** Process the Call ******/
	...
	,	$data				// <str> The API Data that you're using (in JSON form).
	,	$settings = array()	// <str:mixed> Additional settings or instructions to provide.
	)						// RETURNS <mixed> the response of the API call
	
	// $response = self::processCall($apiHost, $apiPath, $data, [$settings]);
	{
		/*
		...
		*/
		...
		var_dump($apiHost . $apiPath, self::$requestMethod, $data);
		...
		else if(self::$requestMethod == "POST")
		{
			...
			,	CURLOPT_POSTFIELDS => array("postData" => $data)
			,	CURLOPT_POST => true
			));
