...
------------------------------------------
------ About the Upload_Image Class ------
------------------------------------------

This class provides a method for uploading images. It will perform several validations on the image to make sure it meets the standards you are looking for during uploading.

Note: do not change the allowed mime types or extensions for the image uploads. These are processed through the Image class, which only accepts the mime types and extensions already provided by this class.
...
if(Form::submitted("upload-image"))
{
	...
	$imageUpload = new Upload_Image($_FILES['image']);
	...
	if($imageUpload->save(APP_PATH . "/assets/images/" . $imageUpload->filename . "." . $imageUpload->extension, Upload_Image::MODE_OVERWRITE))
	{
...
/****** Construct the image uploader ******/
	...
	// $imageUpload = new Upload_Image($_FILES['image'], [$arrayVal]);
	{
