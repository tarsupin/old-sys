...
Saving the text is easy. Just decide on the name of the cache you'd like to save, and then add the data you want to cache. The code for this looks like this:
	...
	Cache_File::save("highlights_today", $textData);
...
Here's an example of how loading a file cache works in practice:
	...
	if(!$cacheData = Cache_File::load("testCache", 15))
	{
		Cache_File::save("testCache", "Save this value: " . md5(mt_rand()));
		
		$cachedData = Cache_File::load("testCache");
	}
...
Cache_File::save($cacheName, $html)

// Loads cache if &lt; $refresh time passed
// A $refresh value of 0 means it will persist indefinitely
Cache_File::load($cacheName, $refresh = 0)


*/

abstract class Cache_File {
...
/****** Save a Cache File ******/
	...
	(
		$cacheName		// <str> The cache name that you'd like to save.
	,	$text			// <str> The text that you'd like to save.
	)					// RETURNS <bool> TRUE on success, FALSE on failure.
	
	// Cache_File::save($cacheName, $text);
	{
...
/****** Return the page to load ******/
	...
	(
		$cacheName		// <str> The name of the cache to load.
	,	$refresh = 0	// <int> The duration (in seconds) that should pass before you refresh the cache.
	,	$output = false	// <bool> TRUE will output the HTML when loading.
	)					// RETURNS <mixed> the text of the cache, or FALSE if the cache is stale (or on failure).
	
	// Cache_File::load($cacheName, $refresh);
	{
		$cacheName = Sanitize::variable($cacheName, ".-");
