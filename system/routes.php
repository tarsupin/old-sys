...
if(count($url) > 0 && $url[0] != "")
{
	...
	/****** Load the Parent Application ******/
	// If the application didn't provide a valid page, load it from the designated parent application instead
	$checkPath = $url_relative;
	
	while(true)
	{
		// Check if the appropriate path exists. Load it if it does.
		if(File::exists(PARENT_APP_PATH . "/controller/" . $checkPath . ".php"))
		{
			require(PARENT_APP_PATH . "/controller/" . $checkPath . ".php"); exit;
		}
